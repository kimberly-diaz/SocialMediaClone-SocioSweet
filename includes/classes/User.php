<?php
	class User {
		...
		public function getFirstQuote() {
			$username = $this->user['username'];
			$query = mysqli_query($this->con, "SELECT quote1 FROM users WHERE username='$username'");
			$row = mysqli_fetch_array($query);
			return $row['quote1'];
		}

		public function getSecondQuote() {
			$username = $this->user['username'];
			$query = mysqli_query($this->con, "SELECT quote2 FROM users WHERE username='$username'");
			$row = mysqli_fetch_array($query);
			return $row['quote2'];
		}
